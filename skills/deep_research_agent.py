...
async def search_github_api(query: str, max_pages: int = 4) -> list[dict]:
    """Search GitHub REST API with pagination and date filtering."""
    repos = []
    async with httpx.AsyncClient(timeout=15, headers=HEADERS) as client:
        for page in range(1, max_pages + 1):
            try:
                # Prioritize recently-updated repos (pushed after 2024-01-01)
                full_query = f"{query} pushed:>2024-01-01"
...
async def scrape_page(url: str, extract_links: bool = False) -> tuple[str, list[str]]:
    """Fetch a web page, extract text and optionally follow links."""
    global _url_counter
...
async def research_category(category: str, description: str, max_urls: int = DEFAULT_MAX_URLS_PER_CATEGORY) -> dict:
    ...
    github_queries = [
        ...
        f"{category} CLI tool 2025",
        f"{category} arch linux",
        f"{category} wayland",
        f"{category} open source",
        f"{category} framework agent",
        f"best {category} terminal",
        f"{category} rust CLI",
        f"{category} lightweight fast",
        f"{category} self hosted",
        f"{category} API python library",
        f"{category} agentic AI 2025 2026",
        f"{category} MCP tool",
        f"{category} LLM integration",
    ...
    # Fetch info for awesome list discoveries
    ...
    ddg_queries = [
        f"best {category} tools linux 2025 2026",
        f"{category} open source comparison review 2025",
        f"{category} arch linux recommendation",
        f"{category} wayland hyprland setup",
        f"{category} alternative free 2025",
        f"top {category} software linux CLI 2025",
        f"{category} python library 2026",
        f"site:wiki.archlinux.org {category}",
        f"site:github.com {category} 2025",
        f"{category} agentic AI framework 2025 2026",
        f"{category} MCP server tools 2025",
        f"{category} LLM agent integration latest",
    ]
    ...
    reddit_posts = await scrape_reddit_search(f"{category} linux 2025")
    reddit_posts += await scrape_reddit_search(f"best {category} arch linux")
    reddit_posts += await scrape_reddit_search(f"{category} agentic AI agent")
    ...
    hn_urls = await search_duckduckgo(f"site:news.ycombinator.com {category} 2025 2026", max_pages=2)
    hn_urls += await search_duckduckgo(f"site:dev.to {category} agent AI 2025", max_pages=2)
    hn_urls += await search_duckduckgo(f"site:medium.com {category} agentic framework 2025", max_pages=2)
...
async def run_full_research(categories: list[dict], max_urls: int = DEFAULT_MAX_URLS_PER_CATEGORY):
    ...
    for i, cat in enumerate(categories):
        ...
        findings = await research_category(cat["name"], cat.get("description", ""), max_urls)
        ...
        with open(md_path, "w") as f:
            f.write(f"# 🔬 Ultra-Deep Research: {cat['name']}\n\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--full", action="store_true", help="Run all 25 categories")
    parser.add_argument("--max-urls", type=int, default=DEFAULT_MAX_URLS_PER_CATEGORY, help="Max URLs per category")
    parser.add_argument("--categories-file", type=str, help="Load categories from JSON file")
    ...
    if args.categories_file:
        ...
        categories = [{"name": topic, "description": topic}]
    else:
        parser.print_help()
        print(f"\nDefault: {len(DEFAULT_CATEGORIES)} categories, ~{DEFAULT_MAX_URLS_PER_CATEGORY} URLs each")
        sys.exit(0)
    
    print(f"🔬 Ultra-Deep Research: {len(categories)} categories, up to {args.max_urls} URLs each")
    print(f"   Estimated total: ~{len(categories) * args.max_urls} URLs")
    
    asyncio.run(run_full_research(categories, max_urls=args.max_urls))
